...
#for i in range(size):
 #line = lines[i].split('\n')[0].split()
 #logn[i] = np.log10(10**(float(line[0]))/(fh*lh))
...
tpl=open('/home/adiman/Desktop/Plots/corrvalues/dx_neg.txt','w')
nsigma=3 
...
for i in range(size):
 DX=0
 #mdv = np.mean(cdvs[np.where((cns>=N[i]-0.1) & (cns<=N[i]+0.1))[0]])
 ...
 for l in range(s):
  ...
  while j<s1-1: #in range(s1-1-5):
   ...
   w2=ws[j+1]
   z2=(w2-lr)/lr
   x1 = (2.0/(3*omm))*(np.sqrt(omm*(1+z1)**3+oml)-1)
   x2 = (2.0/(3*omm))*(np.sqrt(omm*(1+z2)**3+oml)-1)
   ...
   if w1>lr*(1+za) and w1<lr*(1+zb) and vqso<-5000 and np.round(w2-w1,2)<0.06 and j+int(npix/2.0)<=len(ws)-1:
    ...
    sbyn = eqw/unc
    #if sbyn>0 and unc!=0:# and flag==0:
    if unc!=0:# and flag==0:
     #sbyn = eqw/unc 
 ...
 print('DX=',round(DX,2))
 tpl.write(str(round(N[i],1))+'  '+str(DX)+'\n')
